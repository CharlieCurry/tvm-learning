...
if __name__ == '__main__':
    ...
    print("situation1:original /max Y")
    y_max = np.max(Y)
    Y0 = Y / max(y_max, 1e-8)
    X0 = X
    x_train, x_valiation, y_train, y_valiation = train_test_split(X0, Y0, test_size=0.1, random_state=42)
    fit_and_evaluation(x_train, y_train, x_valiation, y_valiation, xgb_params, plan_size, log_interval)

    print("situation2:max-min both")
    X1 = minmax_scale(X)
    Y1 = minmax_scale(Y)
    x_train, x_valiation, y_train, y_valiation = train_test_split(X1, Y1, test_size=0.1, random_state=42)
    fit_and_evaluation(x_train, y_train, x_valiation, y_valiation, xgb_params, plan_size, log_interval)

    print("situation3:maxabs_scale both")
    X2 = maxabs_scale(X)
    Y2 = maxabs_scale(Y)
    ...
    print("situation5:pca-maxabs both")
    pca = PCA(n_components=9)
    ...
    print("situation5_1:pca-maxabs both")
    pca = PCA(n_components=9)
    X4_1 = pca.fit_transform(X)
    Y4_1 = Y / max(y_max, 1e-8)
    #print("pca variance ratio", pca.explained_variance_ratio_)
    x_train, x_valiation, y_train, y_valiation = train_test_split(X4_1, Y4_1, test_size=0.1, random_state=42)
    fit_and_evaluation(x_train, y_train, x_valiation, y_valiation, xgb_params, plan_size, log_interval)
...
Y.shape: (648, 1)
situation0:xgboost
XGB train cost time: 13.7547447681427 	obs: 583 	error:	 0
score: 0.0009096166438441402
situation1:original /max Y
XGB train cost time: 12.550967693328857 	obs: 583 	error:	 0
score: 0.0009096166438441402
situation2:max-min both
XGB train cost time: 7.716375112533569 	obs: 583 	error:	 1
score: 0.0010177815519361315
situation3:maxabs_scale both
XGB train cost time: 12.521536827087402 	obs: 583 	error:	 0
score: 0.0009096166438441402
situation4:pca
XGB train cost time: 6.915027141571045 	obs: 583 	error:	 0
score: 0.0010287857382571318
situation5:pca-maxabs both
XGB train cost time: 6.88860011100769 	obs: 583 	error:	 0
score: 0.0010287857382571318
situation6:pca-maxmin only X
XGB train cost time: 6.93298077583313 	obs: 583 	error:	 0
score: 0.0010287857382571318
situation7:pca-maxabs olny X
XGB train cost time: 6.941955327987671 	obs: 583 	error:	 0
score: 0.0010287857382571318
'''
