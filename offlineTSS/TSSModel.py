...
class TSSModel():
    ...
    def TSS_NN_Model(self,x_train,y_train,x_valiation,y_valiation):
        '''
            :param x_train:
            :param y_train:
            :param x_valiation:
        ...
        for i in range(STEPS):
            ...
            self.sess.run(train_step,feed_dict={self.xs:x_train[start:end], self.ys:y_train[start:end],self.keep_prob: 0.8})
            if i % 2000 == 0:
                print("i=", i, "train_loss=", self.sess.run(hubers_loss, feed_dict={self.xs:x_train,self.ys:y_train,self.keep_prob: 1}))
                print("i=", i, "valiation_loss=", self.sess.run(hubers_loss, feed_dict={self.xs:x_valiation,self.ys:y_valiation,self.keep_prob: 1}))
