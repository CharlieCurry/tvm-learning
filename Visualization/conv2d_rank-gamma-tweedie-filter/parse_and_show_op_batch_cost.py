...
def plot_tuner(data,color,label):
    ...
    plt.plot(dataframe1['iter'][:len(dataframe1)-1], dataframe1['batch_time'].values[:len(dataframe1)-1], color=color, label=label)
...
if __name__ == '__main__':
    ...
    plt.ylabel('each batch(64) cost time(s)', fontsize=12)
    #plt.legend(loc=0, ncol=1, fontsize=6)
    #plt.ylim(200, 500)
    plt.show()
