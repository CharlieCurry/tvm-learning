...
class NNCostModel(CostModel):
    ...
    def __init__(self, task, feature_type, loss_type, num_threads=None, log_interval=25,
                 upper_model=None):
        super(NNCostModel, self).__init__()
    ...
    def fit(self, xs, ys, plan_size):
        ...
        self.tss.TSS_NN_Model(x_train[index],y_train[index])


        logger.debug("NN train: %.2f\tobs: %d\terror: %d\tn_cache: %d",
    ...
    def fit_log(self, records, plan_size):
        ...
        fea_len = len(self.get_feature([0])[0])
    ...
    def predict(self, xs, output_margin=False):
        feas = self.get_feature(xs)

        dtest = self.tss.predict(feas)
    ...
    def spawn_base_model(self):
        return CostModel(self.task, self.fea_type, self.loss_type,
                                self.num_threads, self.log_interval, self)

    def get_feature(self, indexes):
